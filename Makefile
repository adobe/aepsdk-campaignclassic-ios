export EXTENSION_NAME = AEPCampaignClassic
PROJECT_NAME = $(EXTENSION_NAME)
TARGET_NAME_XCFRAMEWORK = $(EXTENSION_NAME).xcframework
SCHEME_NAME_XCFRAMEWORK = AEPCampaignClassic
CURR_DIR := ${CURDIR}
...
check-format:
	swiftformat --lint AEPCampaignClassic/Sources --swiftversion 5.1
	
format:
	swiftformat AEPCampaignClassic/Sources --swiftversion 5.1
...
test: clean
	...
	@echo "### Testing iOS"
	@echo "######################################################################"
	xcodebuild -test-iterations 3 -retry-tests-on-failure test -workspace $(PROJECT_NAME).xcworkspace -scheme $(PROJECT_NAME) -destination $(IOS_DESTINATION) -derivedDataPath build/out -resultBundlePath build/$(PROJECT_NAME).xcresult -enableCodeCoverage YES
...
latest-version:
	(which jq)
	(pod spec cat AEPCampaignClassic | jq '.version' | tr -d '"')

version-podspec-local:
	(which jq)
	(pod ipc spec AEPCampaignClassic.podspec | jq '.version' | tr -d '"')

version-source-code:
	(cat ./AEPCampaignClassic/Sources/CampaignClassicConstants.swift | egrep '\s*EXTENSION_VERSION\s*=\s*\"(.*)\"' | ruby -e "puts gets.scan(/\"(.*)\"/)[0] " | tr -d '"')
